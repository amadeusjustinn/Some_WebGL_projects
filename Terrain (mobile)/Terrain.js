...
class Terrain {
	...
	shapeTerrain() {
		// MP2: Implement this function!
		let cutCount = Math.max(this.div * 2, 75), delta = 0.75, H = Math.random();
		...
		for (let cuts = 0; cuts < cutCount; cuts++) {
			...
			let randNormal = glMatrix.vec3.fromValues(Math.cos(randAngle), Math.sin(randAngle), 0);
			...
			for (let i = 0; i < this.numVertices; i++) {
				...
				else if (glMatrix.vec3.dot(randDifference, randNormal) > 0) currVertex[2] += delta;
